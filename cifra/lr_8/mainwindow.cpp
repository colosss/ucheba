    ...
    : QOpenGLWidget(parent), program(nullptr), rotationAngle(0.0f)
{
    ...
    fogEnabled = true;
    // Сделаем туман более розовым и плотным по умолчанию
    fogColor = QVector3D(1.0f, 0.28f, 0.6f); // розовый
    fogDensity = 0.18f; // усилим экспоненциальный туман
    ...
    // Включаем использование процедурной сетки для тора (переключается клавишей 8)
    torusUseGrid = false;
    torusGridColor = QVector3D(0.1f, 0.9f, 0.9f);
    torusGridScale = 6.0f;
...
// ----------------- Шейдеры (обновлены: добавлено туман, cameraPos и процедурная сетка для тора) -----------------
void OpenGLWidget::setupShaders()
{
    ...
    const char *fshader =
        ...
        "// Параметры для процедурной сетки тора\n"
        "uniform bool torusUseGrid;\n"
        "uniform vec3 torusGridColor;\n"
        "uniform float torusGridScale;\n"
        "out vec4 FragColor;\n"
        "float gridFactor(vec2 uv, float scale) {\n"
        "    // простая полосатая/сеточная маска на основе синусов по x и z\n"
        "    float gx = abs(sin(uv.x * scale));\n"
        "    float gz = abs(sin(uv.y * scale));\n"
        "    // комбинируем чтобы получить пересечения линий\n"
        "    float lines = (1.0 - smoothstep(0.0, 0.15, gx)) + (1.0 - smoothstep(0.0, 0.15, gz));\n"
        ...
        "    // если включена процедурная сетка для тора, наложим её на основании FragPos.xz\n"
        "    if (torusUseGrid) {\n"
        "        // используем мировые координаты x,z как uv для сетки\n"
        "        vec2 uv = FragPos.xz;\n"
        "        float g = gridFactor(uv, torusGridScale);\n"
        "        // смешиваем основной цвет и цвет сетки в местах линий (g>0)\n"
        "        baseColor = mix(baseColor, torusGridColor * 1.2, g);\n"
        "    }\n"
        "    vec4 color = vec4(baseColor, alpha);\n"
        "    if (fogEnabled) {\n"
        "        float dist = length(cameraPos - FragPos);\n"
        "        // экспоненциальный туман (усиленный параметром fogDensity)\n"
...
void OpenGLWidget::keyPressEvent(QKeyEvent *event)
{
    ...
    case Qt::Key_4:
        ...
        break;
    case Qt::Key_5:
        changeSphereAlpha(0.1f);
        break;
    case Qt::Key_6:
        changeSphereAlpha(-0.1f);
        break;
    case Qt::Key_7: // теперь 7 включает/выключает туман
        fogEnabled = !fogEnabled;
        break;
    case Qt::Key_8: // 8 переключает процедрую сетку/цвет у тора
        torusUseGrid = !torusUseGrid;
        break;
    case Qt::Key_T:
        showTorus = !showTorus;
        break;
...
void OpenGLWidget::paintGL()
{
    ...
    // параметры сетки тора
    program->setUniformValue("torusUseGrid", torusUseGrid);
    program->setUniformValue("torusGridColor", torusGridColor);
    program->setUniformValue("torusGridScale", torusGridScale);
    ...
    // Тор
    if (showTorus) {
        // передаём false/true в зависимости от того, хотим ли сетку
        // uniform уже установлен глобально выше (torusUseGrid)
...
// ----------------- Функция создания и отрисовки тора (без текстурных атрибутов — используется процедурная сетка) -----------------
